...
<PropertiesTable
  content={[
    ...
    },
    {
      ...
      type: "Transport",
      isOptional: true,
      description: "Custom transport implementation to use instead of the default HTTP transports. When provided, bypasses automatic transport selection (Streamable HTTP → SSE fallback). Only available for HTTP servers (with 'url'). Useful for custom authentication, alternative protocols, testing, or advanced networking configurations.",
    },
  ]}
/>
...
For advanced use cases, you can provide a custom transport implementation to any HTTP server configuration using the `customTransport` option. This bypasses the automatic transport selection logic.
...
class CustomAuthTransport implements Transport {
  async start() { /* Custom connection logic */ }
  async send(message) { /* Custom message sending */ }
  async close() { /* Custom cleanup */ }
...
const mcpClient = new MCPClient({
  servers: {
    customServer: {
      ...
      customTransport: new CustomAuthTransport(),
    },
  },
});
```
...
- **Custom Authentication**: Implement specialized authentication flows
- **Alternative Protocols**: Use WebSocket, custom streaming protocols, etc.
- **Testing**: Create mock transports for predictable testing
- **Advanced Networking**: Custom retry logic, connection pooling, etc.

### Behavior

When `customTransport` is provided:
- Automatic transport selection (Streamable HTTP → SSE fallback) is bypassed
- Standard HTTP options (`requestInit`, `eventSourceInit`, `authProvider`) are ignored
- The custom transport handles all communication
- Client-level options like `timeout` are still respected

**Note**: `customTransport` is only available for HTTP server configurations (those with `url`). It cannot be used with Stdio servers (those with `command`).
